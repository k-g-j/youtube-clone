...
const rawVideoBucketName = 'kgj-yt-raw-videos';
const processedVideoBucketName = 'kgj-yt-processed-videos';

const localRawVideoPath = './raw-videos';
const localProcessedVideoPath = './processed-videos';

/**
 * Creates the local directories for raw and processed files
 */
...
export function convertVideo(
  ...
  return new Promise<void>((resolve, reject) => {
    ffmpeg(`${localProcessedVideoPath}/${rawVideoName}`)
      .outputOptions('-vf', 'scale=2*trunc((iw*360/ih)/2):360')
      // scaled to 360p
      .on('command', (cmd) => {
